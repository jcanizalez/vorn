...
export function LaunchAgentNode({ label, config, selected, executionStatus, onClick }: Props) {
  ...
  const promptPreview = config.prompt
    ? config.prompt.length > 60 ? config.prompt.slice(0, 60) + '...' : config.prompt
  ...
  const statusColor = executionStatus === 'running' ? 'border-yellow-500'
    : executionStatus === 'success' ? 'border-green-500'
    : executionStatus === 'error' ? 'border-red-500'
    : selected ? 'border-blue-500' : 'border-white/[0.12]'

  const statusBg = executionStatus === 'running' ? 'bg-yellow-500/10'
    : executionStatus === 'success' ? 'bg-green-500/10'
    : executionStatus === 'error' ? 'bg-red-500/10'
    : selected ? 'bg-blue-500/10' : 'bg-[#232328]'
